...
router.post("/full", (req, res) => {
  console.log("/order_meal/full body:", req.body);
  ...
  const insertOrderQuery =
    "INSERT INTO orders (order_date, status, user_id, first_name, last_name, phone, email, id_number, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
  db.query(insertOrderQuery, [order_date, status, user_id, first_name || null, last_name || null, phone || null, email || null, id_number || null, address || null], (err, result) => {
    ...
    db.query(insertItemsQuery, [values], (err2) => {
      if (err2)
        return res.status(500).json({ message: "Error adding items", error: err2 });

      res.json({ message: "Order and items created successfully", orderId });
    });
  });
});
